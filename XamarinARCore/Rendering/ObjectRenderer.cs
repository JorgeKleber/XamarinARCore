...
namespace XamarinARCore.Rendering
{
	...
	{
		private static string TAG = typeof(ObjectRenderer).Name;
		...
		*/
		...
		// Shader names.
		private static string VERTEX_SHADER_NAME = "shaders/ar_object.vert";
		private static string FRAGMENT_SHADER_NAME = "shaders/ar_object.frag";

		private static int COORDS_PER_VERTEX = 3;
		private static float[] DEFAULT_COLOR = new float[] { 0f, 0f, 0f, 0f };

		// Note: the last component must be zero to avoid applying the translational part of the matrix.
		private static float[] LIGHT_DIRECTION = new float[] { 0.250f, 0.866f, 0.433f, 0.0f };
		...
		// Set some default material properties to use for lighting.x
		...
		private static string USE_DEPTH_FOR_OCCLUSION_SHADER_FLAG = "USE_DEPTH_FOR_OCCLUSION";
        ...
        */
		public void createOnGlThread(Context context, string objAssetName, string diffuseTextureAssetName)
		{
			...
			// Read the obj file.
			System.IO.Stream objInputStream = context.Assets.Open(objAssetName);
			...
			int totalBytes = normalsBaseAddress + 4 * normals.Limit();
			...
			GLES20.GlBufferSubData(GLES20.GlArrayBuffer, verticesBaseAddress, 4 * vertices.Limit(), vertices);
			GLES20.GlBufferSubData(GLES20.GlArrayBuffer, texCoordsBaseAddress, 4 * texCoords.Limit(), texCoords);
			GLES20.GlBufferSubData(GLES20.GlArrayBuffer, normalsBaseAddress, 4 * normals.Limit(), normals);
			GLES20.GlBindBuffer(GLES20.GlArrayBuffer, 0);

			// Load index buffer
			GLES20.GlBindBuffer(GLES20.GlElementArrayBuffer, indexBufferId);
			indexCount = indices.Limit();
			GLES20.GlBufferData(GLES20.GlElementArrayBuffer, 2 * indexCount, indices, GLES20.GlStaticDraw);
		...
		*/
		...
		private void compileAndLoadShaderProgram(Context context)
		{
			...
			Dictionary<string, int> defineValuesMap = new Dictionary<string, int>();
			...
			int fragmentShader = ShaderUtil.loadGLShader(TAG, context, GLES20.GlFragmentShader, FRAGMENT_SHADER_NAME, defineValuesMap);
		...
		*/
		public void updateModelMatrix(float[] modelMatrix, float scaleFactor)
		{
			...
			Android.Opengl.Matrix.MultiplyMM(this.modelMatrix, 0, modelMatrix, 0, scaleMatrix, 0);
		}
		...
		*/
		...
		public void draw(float[] cameraView,float[] cameraPerspective,float[] colorCorrectionRgba,float[] objColor)
		{
			...
			Android.Opengl.Matrix.MultiplyMM(modelViewMatrix, 0, cameraView, 0, modelMatrix, 0);
			Android.Opengl.Matrix.MultiplyMM(modelViewProjectionMatrix, 0, cameraPerspective, 0, modelViewMatrix, 0);

			GLES20.GlUseProgram(program);

			// Set the lighting environment properties.
			Android.Opengl.Matrix.MultiplyMV(viewLightDirection, 0, modelViewMatrix, 0, LIGHT_DIRECTION, 0);
			...
			GLES20.GlVertexAttribPointer(texCoordAttribute, 2, GLES20.GlFloat, false, 0, texCoordsBaseAddress);
